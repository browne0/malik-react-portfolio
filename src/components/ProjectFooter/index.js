...
const ProjectFooter = props => {
  ...
  let style = {
    container: {
      backgroundColor: props.currentProject.color ? props.currentProject.color : props.currentProject.bgColor,
      minHeight: props.currentProject.name === 'Spotter' ? '40vh' : '30vh'
    }
  }
  let className = props.nextProject.name === 'Spotify Mixmax Integration' ? null : 'pic'
  let path = {
      ...
      <div className="footer-container">
        ...
        <div className="next-project-image">
          <div className="inner">
            <p>Up Next:</p>
            <Link to={path}>
            <img className={className} src={props.nextProject.image_urls.screenshots[0]} alt={props.nextProject.name} />
            </Link>
          </div> 
        </div>
      </div>
